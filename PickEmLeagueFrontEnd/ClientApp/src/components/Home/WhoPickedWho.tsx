...
export function WhoPickedWho() {
    ...
    function getWhoPickedGameResult(game: Game, result: GameResult) {
        return gamePicks.filter(gp => gp.game?.id == game.id && gp.pick === result)
            .map(gp => gp.user);
    }
                ...
                <Row>
                    <Col className="col-6 text-center">
                        ...
                        {getFinishedGames().map(g => (
                            <Row key={g.id}>
                                <Col className="col-5">
                                    <CollapsibleTeamAndPicks users={getWhoPickedGameResult(g, GameResult.AwayWin)} game={g} result={GameResult.AwayWin} />
                                </Col>
                                <Col className="col-2">
                                    <h4>@</h4>
                                </Col>
                                <Col className="col-5">
                                    <CollapsibleTeamAndPicks users={getWhoPickedGameResult(g, GameResult.HomeWin)} game={g} result={GameResult.HomeWin} />
                                </Col>
                            </Row>
                        ))}
                    </Col>
...
function CollapsibleTeamAndPicks(props: CollapsibleTeamAndPicksProps) {
    ...
    return (
        props.result === GameResult.AwayWin ?
            (
                <Container>
                    <Row>
